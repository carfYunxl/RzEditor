...
int gSocketType         = SOCK_STREAM;              // default to TCP socket type
int gProtocol           = IPPROTO_TCP;              // default to TCP protocol
int gBufferSize         = DEFAULT_BUFFER_SIZE;
int gInitialAccepts     = DEFAULT_OVERLAPPED_COUNT;
int gMaxAccepts         = MAX_OVERLAPPED_ACCEPTS;
int gMaxReceives        = MAX_OVERLAPPED_RECVS;
int gMaxSends           = MAX_OVERLAPPED_SENDS;
char* gBindAddr         = NULL;                     // local interface to bind to
const char* gBindPort   = "5150";                   // local port to bind to
...
volatile LONG gConnections      = 0;
volatile LONG gConnectionsLast  = 0;
volatile LONG gOutstandingSends = 0;
...
typedef struct _SOCKET_OBJ
{
    ...
    int                 bClosing;           // Is the socket closing?
...
CRITICAL_SECTION gSocketListCs;
CRITICAL_SECTION gPendingCritSec;
...
BUFFER_OBJ* gPendingSendListEnd = NULL;
...
void dbgprint(const char* format, ...)
{
    ...
    char    dbgbuf[2048];
    va_start(vl, format);
    wvsprintf((LPWSTR)dbgbuf, (LPCWSTR)format, vl);
    va_end(vl);
    printf(dbgbuf);
...
void PrintStatistics()
{
    ...
    ULONGLONG elapsed = (tick - gStartTime) / 1000;
    if (elapsed == 0)
        return;
    printf("\n");
    ...
    gStartTimeLast = static_cast<LONG>(tick);
}
// Function: PostRecv
...
int main(int argc, char** argv)
{
    ...
    HANDLE              WaitEvents[MAX_COMPLETION_THREAD_COUNT];
    HANDLE              hrc;
    ...
    while (ptr)
    {
        ...
        PrintAddress(ptr->ai_addr, static_cast<int>(ptr->ai_addrlen));
        printf("\n");
    ...
    freeaddrinfo(res);
    gStartTime = gStartTimeLast = static_cast<LONG>(GetTickCount64());
    interval = 0;
    while (1)
    {
